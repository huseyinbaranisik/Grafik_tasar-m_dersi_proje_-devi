...
const navItems: NavItem[] = [
  ...
  },
  {
    name: "Akademik",
    items: [
      { name: "Müfredat", href: "/courses", desc: "Lisans eğitim planı ve dersler" },
...
const socialLinks = [
  ...
  {
    name: "LinkedIn",
    href: "https://www.linkedin.com/school/samsununiversitesi/posts/?feedView=all",
    icon: (
      <svg viewBox="0 0 24 24" className="w-4 h-4" fill="currentColor">
  ...
  {
    name: "YouTube",
    href: "https://www.youtube.com/c/SamsunÜniversitesii",
    icon: (
      <svg viewBox="0 0 24 24" className="w-4 h-4" fill="currentColor">
...
function DropdownMenu({ items, onItemClick }: { items: DropdownItem[]; onItemClick: (href: string) => void }) {
  return (
            ...
            {navItems.map((item) => {
              ...
              return (
                <div key={item.name} className="relative">
                  ...
                  {isOpen && item.items && (
                    <DropdownMenu items={item.items} onItemClick={handleLinkClick} />
                  )}
                </div>
              );
            })}
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            ...
            <div>
              ...
              <ul className="space-y-3">
                {[
                  ...
                  <li key={l.name}><Link href={l.href} onClick={() => handleLinkClick(l.href)} className="text-sm text-muted-foreground hover:text-foreground transition-colors">{l.name}</Link></li>
                ))}
              </ul>
            </div>
          ...
          <div className="border-t border-border pt-8 flex flex-col sm:flex-row justify-between items-center gap-4">
            ...
            <div className="flex gap-5 text-xs text-muted-foreground">
              <Link href="/about" onClick={() => handleLinkClick("/about")} className="hover:text-foreground transition-colors">Hakkında</Link>
              <Link href="/contact" onClick={() => handleLinkClick("/contact")} className="hover:text-foreground transition-colors">İletişim</Link>
